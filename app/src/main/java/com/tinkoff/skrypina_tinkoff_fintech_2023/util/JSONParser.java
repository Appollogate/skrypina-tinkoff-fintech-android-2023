...
public class JSONParser {
    public static List<MoviePreviewCard> getTopMoviesList(String json) {
        JsonObject jsonObject = JsonParser.parseString(json).getAsJsonObject();
        JsonArray films = jsonObject.getAsJsonArray("films");
        List<MoviePreviewCard> result = new ArrayList<>();
        for (int i = 0; i < films.size(); ++i) {
            ...
            result.add(new MoviePreviewCard(posterImageURL, name, year, genre));
        }
        return result;
    }
}
